...
class MergePDFFileFirst(StatesGroup):
    ...
    texts = {"MergePDFFileFirst:input2": "Upload your second file"}


@user_handlers_router.message(CommandStart())
async def start_cmd(message: types.Message) -> None:
    await message.answer(
        "Hi, I am your PDF converter assistant", reply_markup=INITIAL_KEYBOARD
    )
...
async def document_without_command(message: types.Message, state: FSMContext):
    ...
    if file_name_output_temp[1] != ".pdf":
        if file_name_output_temp[1] not in SUPPORTED_FILES_LIST:
            ...
            await message.answer(
                "Please select what you want to do with this file",
...
@user_handlers_router.callback_query(
    StateFilter(None), F.data.contains("topdf_file_first")
)
...
@user_handlers_router.callback_query(
    StateFilter(None), F.data.contains("compress_file_first")
)
...
@user_handlers_router.callback_query(
    StateFilter(None), F.data.contains("rotate_file_first")
)
async def rotate_file_first(callback: types.CallbackQuery, state: FSMContext):

    await callback.message.answer(
        "Select how you want to rotate your PDF", reply_markup=ROTATE_KEYBOARD
    )
    await state.set_state(Rotate.selectOption)


@user_handlers_router.callback_query(
    StateFilter(None), F.data.contains("merge_file_first")
)
...
async def cancel_handler(message: types.Message, state: FSMContext) -> None:
    ...
    await state.clear()
    await message.answer(
        "Your actions were cancelled. Let's start it over",
        reply_markup=INITIAL_KEYBOARD,
    )
...
async def topdf_Input(message: types.Message, state: FSMContext):
    ...
    if file_name_output_temp[1] == ".pdf":
        await message.answer(
            "Your file is already in PDF format \n Please try another file"
        )
        await state.set_state(ToPDF.input)
        return
...
async def rotatePDF_Input_Improper(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        "Your response is not a PDF document \n Please upload a PDF document"
    )
    return
